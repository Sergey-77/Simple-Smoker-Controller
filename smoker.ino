...
void loop() {
  ...
  if(run == 1 && running == 1 ){
    elapsedTime = time_now - startTime;
    ...
    if(elapsed_time_h == cooking_time_hours && elapsed_time_m >= cooking_time_minutes){
      run = 0;
      running = 0;
    }
  }
  ...
  if(running){
    millis_buffer = time_now - pwm_start;
  ...
  if ((time_now - temp_update_time) > 5000UL){
    ...
    temp_update_time = time_now;
  }
  if(settings.pid_enabled && running && ((time_now - pid_update_time) > 30000UL)){
    pwm_duty_cycle = heater_controll_pid(smoker_temp_setpoint, thermocouple);
    pwm_on_phase = PWM_LENGTH * pwm_duty_cycle / 100;
    pid_update_time = time_now;
  } 
}
